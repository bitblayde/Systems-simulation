...
int main(int argc, char **argv){
  ...
  if(argc == 1){
      ...
      y = atoi(argv[2]);
      veces = atoi(argv[3]);
      apartado = atoi(argv[4]);
  }else if(argc == 6){
    x = atoi(argv[1]);
    y = atoi(argv[2]);
    veces = atoi(argv[3]);
    apartado = atoi(argv[4]);
    ejecuciones = atoi(argv[5]);
  }else{
    cout << "\nFormato erróneo de argumentos. ";
    cout << "\nPrimera opción. 5 argumentos: <X> <Y> <Veces> <Apartado>\n";

    cout << "\nSegunda opción. 6 argumentos: <X> <Y> <Veces> <Apartado> <Ejecuciones>\n";
    exit(1);
  }
