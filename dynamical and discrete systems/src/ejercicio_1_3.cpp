...
//printf("\nTiempo medio de estancia en el sistema bis= %.3f",acum_cola/atendidos+tserv);
	if(!mostrar_por_pantalla){
		...
		total_desv.emplace_back(maximacola);

		med[0] += retrasomedio;
		med[1] += estanciamedia;
		med[2] += encolamedio;
		med[3] += ensistemamedio;
		med[4] += porcentajemedioocio;
		med[5] += maximacola;
		...
		desv[4][iteracion] = porcentajemedioocio;
		desv[5][iteracion] = maximacola;
	}

}

void init_desv(int ejecuciones){
	for(auto i = 0; i < ejecuciones; i++){
		desv.emplace_back(vector<float>(ejecuciones));
	}
}

void calcula_desv(int ejecuciones){
	for (auto i = 0; i < ejecuciones; i++){
		desv[0][i] = (ejecuciones*(med[0]*med[0])) - (desv[0][i] * desv[0][i]);
		desv[1][i] = (ejecuciones*(med[1]*med[1])) - (desv[1][i] * desv[1][i]);
		desv[2][i] = (ejecuciones*(med[2]*med[2])) - (desv[2][i] * desv[2][i]);
		desv[3][i] = (ejecuciones*(med[3]*med[3])) - (desv[3][i] * desv[3][i]);
		desv[4][i] = (ejecuciones*(med[4]*med[4])) - (desv[4][i] * desv[4][i]);
		desv[5][i] = (ejecuciones*(med[5]*med[5])) - (desv[5][i] * desv[5][i]);

		total_desv[i] = (ejecuciones*pow(total_med, 2)) - (total_desv[i]*total_desv[i]);
...
void monitor(){
	...
	nodo.retraso = nada;
	insertar_lsuc(nodo);
	cout<<"tiempo y media en sistema "<<reloj<< \
			" "<<(acum_sistema+(reloj - tultsuc_sistema) * ensistema)/reloj<<endl;
}

/* Procedimiento suceso */
void suceso(int ejecuciones){
	switch(nodo.suceso){
		case suceso_llegada: llegada(); break;
    case suceso_salida: salida(); break;
    case suceso_finsimulacion: fin(ejecuciones); break;
...
int main(int argc, char *argv[]){
  ...
  if (argc != 5 && argc !=6){
	...
	if (mostrar_por_pantalla){
		...
		float acc = (float) 1 /(ejecuciones - 1) ;

		calcula_desv(ejecuciones);

		desv[0][0] = sqrt(res[0]*acc);
		desv[1][0] = sqrt(res[1]*acc);
		desv[2][0] = sqrt(res[2]*acc);
		desv[3][0] = sqrt(res[3]*acc);
		desv[4][0] = sqrt(res[4]*acc);
		desv[5][0] = sqrt(res[5]*acc);


		res[0] = sqrt(res[0]*acc);
		res[1] = sqrt(res[1]*acc);
		res[2] = sqrt(res[2]*acc);
		res[3] = sqrt(res[3]*acc);
		res[4] = sqrt(res[4]*acc);
		res[5] = sqrt(res[5]*acc);
		...
		cout << "Desviación Típica. Tiempo medio de espera en cola " << res[0] << endl;
		cout << "Desviación Típica. Tiempo medio de estancia en el sistema " << res[1] << endl;
		cout << "Desviación Típica. Numero medio de clientes en cola " << res[2] << endl;
		cout << "Desviación Típica. Numero medio de personas en el sistema " << res[3] << endl;
		cout << "Desviación Típica. Longitud media de colas no vacias " << res[4] << endl;
		cout << "Desviación Típica. Porcentaje medio de tiempo de ocio por servidor " << res[5] << endl;
