...
void fin_carga()
{
  ...
  if ((remolcador == LIBRE) && (tormenta == false)) //comienza a desatracar el barco
    ...
    if (nodo.reg_cola.tipo == 0){
        toneladas_cargadas = toneladas_cargadas + 1000.0f;
    }
    else if(nodo.reg_cola.tipo == 1){
      toneladas_cargadas = toneladas_cargadas + 2000.0f;
    }
    else if(nodo.reg_cola.tipo == 2){
        toneladas_cargadas = toneladas_cargadas + 3000.0f;
    }
...
void generador_informes(int simulaciones)
{
  ...
  printf("%.2f",media[11]);
  //printf("\nTotal de toneladas cargadas: media(%.2f), dt(%.2f)\n\n",media[11],dt[11]);
}
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 2){
    ...
    sscanf(argv[1],"%d",&simulaciones);
    sscanf(argv[2],"%d",&remolcadores);
    sscanf(argv[3],"%d",&num_atraques);
  }
  else{
    printf("\n\nFormato Argumentos -> <numero_simulaciones>\n\n");
    printf("\n\nFormato Argumentos -> <numero_simulaciones> <remolcadores> <num_atraques> \n\n");
  	exit(1);
  }
  sscanf(argv[1],"%d",&simulaciones);
